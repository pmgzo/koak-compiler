...
findTrickyError listXps = checkErrXp (protos ++
    [(Protof (Typed "printInt" INT) [(Typed "nb" INT)] (Exprs [])),
    (Protof (Typed "printDouble" DOUBLE) [(Typed "nb" DOUBLE)] (Exprs []))])
    listXps NULL ++ checkBinOperation listXps
